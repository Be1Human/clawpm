...
export function createMcpServer(options?: { agentId?: string; memberIdentifier?: string; principal?: AuthPrincipal | null }) {
  ...
  mcp.tool('create_task', '创建需求节点（只需 title，其余可选）', {
    ...
    owner: z.string().optional().describe('负责人（Owner，决策者）'),
    assignee: z.string().optional().describe('处理人（Assignee，执行者）'),
    collaborators: z.array(z.string()).optional().describe('协作人员标识数组'),
  ...
  mcp.tool('update_task', '更新节点信息', {
    ...
    status: z.enum(['backlog', 'planned', 'active', 'review', 'done']).optional(),
    priority: z.string().optional(),
    owner: z.string().optional().describe('负责人（Owner）'),
    assignee: z.string().optional().describe('处理人（Assignee）'),
    collaborators: z.array(z.string()).optional().describe('协作人员标识数组'),
