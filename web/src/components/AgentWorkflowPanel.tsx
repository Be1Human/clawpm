  ...
  return (
    <section className="rounded-xl border border-slate-200 bg-white overflow-hidden">
      <div className="px-4 py-3 border-b border-slate-100 flex items-start justify-between gap-3">
        <div>
          <h3 className="text-xs text-slate-500 uppercase tracking-wider font-semibold">Agent 工作流</h3>
          <p className="text-[11px] text-slate-400 mt-1">拆分 → 领取 → 执行 → 测试 → 验收</p>
        </div>
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="space-y-2">
          <label className="text-[11px] font-medium text-slate-500">执行 Agent</label>
          <div className="flex gap-2">
            <input
              ...
              className="min-w-0 flex-1 border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-500/20 disabled:bg-slate-50"
            ...
            {!task.claimActive && task.status !== 'done' ? (
              ...
              <button
                ...
                className="px-3 py-2 text-xs rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50 disabled:opacity-40 whitespace-nowrap"
              >{pending === 'release' ? '释放中...' : '释放'}</button>
            ) : null}
          </div>
          {task.claimActive && <p className="text-[10px] text-slate-400">租约到期：{new Date(task.claim.leaseUntil).toLocaleString('zh-CN')}</p>}
        </div>
        ...
        <details className="group border-t border-slate-100 pt-3" open={nextAction.action === 'define_acceptance'}>
          <summary className="cursor-pointer list-none flex items-center justify-between text-sm font-medium text-slate-700">
            <span>验收标准</span><span className="text-slate-300 group-open:rotate-90 transition-transform">›</span>
          </summary>
          <div className="mt-3 space-y-2">
            ...
            <button
              ...
              className="px-3 py-1.5 text-xs rounded-lg bg-slate-800 text-white hover:bg-slate-900 disabled:opacity-40"
        ...
        <details className="group border-t border-slate-100 pt-3" open={nextAction.action === 'test'}>
          <summary className="cursor-pointer list-none flex items-center justify-between text-sm font-medium text-slate-700">
            <span>测试任务与证据</span><span className="text-slate-300 group-open:rotate-90 transition-transform">›</span>
          </summary>
          <div className="mt-3 space-y-2">
            {tests.slice().reverse().map((result: any) => (
              <div key={result.id} className="rounded-lg bg-slate-50 border border-slate-100 px-3 py-2 text-xs">
                <div className="flex justify-between gap-2">
                  ...
                  <span className="text-slate-400">{result.actor}</span>
                </div>
                {result.command && <code className="block mt-1 text-[10px] text-slate-500 break-all">{result.command}</code>}
              </div>
            ))}
            <div className="grid grid-cols-[110px_1fr] gap-2">
              <select value={testStatus} onChange={event => setTestStatus(event.target.value as 'passed' | 'failed')}
                disabled={!canEdit || task.status === 'done'} className="border border-slate-200 rounded-lg px-2 py-2 text-sm bg-white">
              ...
              <input value={testCommand} onChange={event => setTestCommand(event.target.value)} disabled={!canEdit || task.status === 'done'}
                className="border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none" placeholder="测试命令，如 pnpm test" />
            </div>
            <input value={testSummary} onChange={event => setTestSummary(event.target.value)} disabled={!canEdit || task.status === 'done'}
              className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none" placeholder="测试摘要" />
            <textarea value={testEvidence} onChange={event => setTestEvidence(event.target.value)} disabled={!canEdit || task.status === 'done'} rows={2}
              className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none" placeholder="证据路径或日志摘要，每行一条" />
            <button
              ...
              className={cn('px-3 py-1.5 text-xs rounded-lg text-white disabled:opacity-40', testStatus === 'passed'
          ...
          {task.status === 'done' ? (
            ...
            <div className="space-y-2">
              <input value={completionSummary} onChange={event => setCompletionSummary(event.target.value)} disabled={!canEdit}
                className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none" placeholder="完成摘要" />
              <textarea value={completionEvidence} onChange={event => setCompletionEvidence(event.target.value)} disabled={!canEdit} rows={2}
                className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none" placeholder="交付证据，每行一条：提交、代码路径、测试命令或安装包" />
              <button
                ...
                className="w-full px-3 py-2 text-xs rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-40"
