...
> - v1.4 (2026-03-01): **需求图谱重设计** — 废弃固定四级结构，改为无限深度自由节点 + 标签分类 + 关联图谱
...
## 十一、需求图谱视图增强（v1.3 → v1.4 持续迭代）

### 11.1 过滤器（v1.3 已实现，v1.4 扩展标签过滤）

在需求树/图谱页面顶部增加过滤器：

| 过滤器 | 选项 |
|--------|------|
| 标签 | 全部 / epic / feature / bug / spike...（v1.4 新增） |
...
**场景：** 用户想在某个节点下穿插添加一个新子节点，或将已有节点挂入另一个节点。

**入口：**
1. **图谱行内** — 每行右侧 `+` 按钮 → 在该节点下创建子节点（已实现）
2. **全局创建弹窗** — 支持搜索并选择父节点（已实现）
3. **拖拽迁移** — 在思维导图中拖拽节点到新父节点上，确认后执行迁移（v1.4 新增）

### 11.3 视图形态

| 视图 | 特点 | 适合场景 |
|------|------|---------|
| **树形列表** | 缩进列表，信息密集 | 查看状态/进度/负责人/标签 |
| **思维导图** | 节点连线，空间可视化，含关联线 | 整理结构/依赖分析/头脑风暴 |

### 11.4 思维导图关联线控制（v1.4 新增）

| 控制 | 描述 |
|------|------|
| 关联线类型开关 | 工具栏复选框：显示/隐藏 阻塞依赖 / 顺序依赖 / 弱关联 |
| 添加关联 | 右键节点 → "添加关联" → 选择目标 → 选类型 |
| 关联线动效 | 阻塞依赖线在被阻塞方状态非 done 时，源节点角标显示警告 |
| 节点迁移 | 拖拽到新父节点 bounding box 触发迁移确认对话框 |

**技术选型：** `@xyflow/react`（原 react-flow），MIT 协议，支持拖拽、缩放、自定义节点、自定义边。

---

## 九、需求图谱（v1.4 重设计）

> ⚠️ **本节替代 v1.1 的"需求树"章节**。废弃固定四级结构（Epic/Story/Task/Subtask），改为"自由节点 + 标签分类 + 关联图谱"模型。

### 9.0 设计理念

产品需求天然具有**分形（Fractal）特性**：一个需求可以无限拆解，拆解后的子需求本质上和父需求是同类的——都是"一件需要完成的事"，只是粒度不同。强行规定"第一层叫 Epic、第二层叫 Story"会在实际使用中产生认知摩擦：

- 一个大型 Feature 下面可能直接挂 Task，中间并不需要 Story
- 一个 Bug 修复可能直接挂在顶层，没必要包一层 Epic
- 跨模块的需求可能在多个地方出现关联，但在树里只能挂一个父节点

**新设计的核心原则：**
1. **节点平等** — 所有需求节点本质相同，都有 title / status / labels / owner / progress
2. **无限深度** — 任意节点都可以有任意数量的子节点，没有层级限制
3. **标签分类** — 用自定义标签（labels）替代固定类型（type），如 `#epic`、`#bug`、`#feature`
4. **双层结构** — 层级（树状亲子关系）+ 关联（跨节点依赖/关系）
5. **节点可迁移** — 可将任意节点（及其子树）移入另一个父节点
...
```
需求节点（Requirement Node）
├── id          唯一标识（如 U-001）
├── title       标题
├── description 详细描述
├── status      状态：planned / active / review / blocked / done
├── progress    进度 0–100
├── labels      自定义标签（数组），如 ["epic", "用户系统", "P0"]
├── owner       负责人
├── parent_id   父节点 ID（null = 顶层根节点）
├── position    在思维导图中的坐标（x, y）
...
|----------|------------|------|
| `epic` | Epic | 顶层大功能方向 |
| `feature` | Story | 一个完整的用户可见特性 |
| `task` | Task | 可执行的开发工作单元 |
| `bug` | — | 缺陷修复 |
| `spike` | — | 技术探索/预研 |
| `chore` | — | 工程维护、重构 |

> **与旧设计的关键区别：** 标签只是分类标注，不决定节点能否有子节点，也不约束层级深度。一个 `task` 标签的节点完全可以有子节点。

---

### 9.2 关联类型（Association Types）

除了亲子关系（树状层级），节点之间还支持跨节点的"关联"，构成需求图谱（DAG）：

```
节点 A ──────────────────────────────── 节点 B

  树状亲子: A 是 B 的父节点（实线，灰色）
  阻塞依赖: A 被 B 阻塞（B 必须先完成，红色虚线带箭头 →）
  顺序依赖: 建议先做 B 再做 A（橙色虚线带箭头 →）
  弱关联:   A 与 B 主题相关（蓝色虚线，无箭头）
```

| 关联类型 | 英文 | 方向 | 语义 | 思维导图显示 |
|----------|------|------|------|------------|
| **阻塞依赖** | `blocks` | 单向（B→A，B 阻塞 A） | B 未完成，A 无法开始 | 红色虚线 + 箭头（粗）|
| **顺序依赖** | `precedes` | 单向（B→A） | 推荐先做 B 再做 A，不强制阻塞 | 橙色虚线 + 箭头（细）|
| **弱关联** | `relates` | 双向（对称） | 主题/领域相关，无依赖 | 蓝色虚线（无箭头）|

**关联规则：**
1. **循环依赖检测**：阻塞/顺序依赖不允许形成环路（A→B→C→A 会被系统拒绝），系统在创建时自动检测。
2. **关联可见性**：每条关联线可独立显示/隐藏，思维导图默认显示阻塞依赖，隐藏弱关联。
3. **一个节点可有多个关联**：多对多，不限数量。
4. **关联与树层独立**：移动节点（换父节点）时，关联关系保持不变。
...
| 操作 | 描述 |
|------|------|
| **创建子节点** | 在任意节点下创建子节点（无类型限制） |
| **迁移节点** | 将节点（含整个子树）移入新父节点，原父子关系断开 |
| **添加关联** | 在两个节点之间创建阻塞/顺序/弱关联 |
| **删除关联** | 断开两个节点的关联（不影响树层结构） |
| **更换父节点** | 拖拽到新父节点，子树跟随迁移 |
| **提升为根节点** | 断开父节点，成为顶层节点 |
...
| 功能 | 描述 | 优先级 |
|------|------|--------|
| 节点自由拖拽 | 改变显示坐标（位置），不影响亲子关系 | P0 |
| 拖拽到新父节点 | 拖入另一个节点内部区域，触发"迁移"确认 | P1 |
| 关联线可见性切换 | 按关联类型批量显示/隐藏 | P0 |
| 新建子节点 | 右键 / `+` 按钮 | P0 |
| 添加关联 | 右键菜单"添加关联"→选择目标节点→选类型 | P1 |
| 节点折叠 | 双击折叠/展开子树 | P0 |
| 标签色标 | 按第一个标签决定节点主色 | P0 |
| 关联状态感知 | 若阻塞方未完成，被阻塞节点自动标注警告 | P1 |
...
| ID | 功能 | 描述 | 优先级 |
|----|------|------|--------|
| RG-01 | 自由树节点 | 任意节点可有任意数量子节点，无深度限制 | P0 |
| RG-02 | 标签系统 | 节点支持多标签，系统提供推荐标签，支持自定义 | P0 |
| RG-03 | 节点迁移 | 将节点及子树移入新父节点 | P1 |
| RG-04 | 阻塞依赖关联 | 创建单向阻塞关联，阻止循环依赖 | P1 |
| RG-05 | 顺序依赖关联 | 创建单向顺序推荐关联 | P2 |
| RG-06 | 弱关联 | 创建双向主题关联 | P1 |
| RG-07 | 关联可见性 | 按类型显示/隐藏思维导图中的关联线 | P1 |
| RG-08 | 循环依赖检测 | 创建依赖关联时自动检测并拒绝循环依赖 | P1 |
| RG-09 | 过滤器 | 按标签/状态/负责人/里程碑过滤可见节点 | P1 |
| RG-10 | 关联状态感知 | 阻塞依赖的源节点未完成时，目标节点显示警告 | P2 |
