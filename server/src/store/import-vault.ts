// 文本 vault → SQLite 导入。
// 建一个全新的最小库（仅需求树相关的 10 张表，DDL 与 db/schema.ts 对齐；
// 真实应用打开时 connection.ts 的增量迁移会补齐其余列/表）。
// 用途：round-trip 验证（export→import→export 字节一致）与老架构回迁。
...
export function importVault(opts: ImportOptions): ImportReport {
  ...
  try {
    ...
    const run = db.transaction(() => {
      const now = nowSqlite();
      db.prepare('INSERT INTO projects (id, slug, name) VALUES (1, ?, ?)').run(
        slug,
        vault.config.name
      );
      ...
      const insDomain = db.prepare(
        'INSERT INTO domains (project_id, name, task_prefix, keywords, color) VALUES (1, ?, ?, ?, ?)'
      );
      ...
      const insMilestone = db.prepare(
        'INSERT INTO milestones (project_id, name, target_date, status, description) VALUES (1, ?, ?, ?, ?)'
      );
