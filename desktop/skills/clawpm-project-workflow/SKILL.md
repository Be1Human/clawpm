...
创建、拆分、改写或领取任务前，确保 `description` 使用以下 Markdown 结构；缺少任一必填段落时不得写入新任务或领取现有任务：

```markdown
### 背景
用 1–2 句话说明现状、问题来源以及为什么需要这个任务。

### 目标
用 1 句话说明本任务完成后可观察的单一结果。

### 涉及文档
- `docs/example.md` — 本任务读取、修改或产出的原因

### 范围与约束
- 仅在确有必要时补充，最多 3 条；写清包含、不包含和硬约束。
```

1. `背景`、`目标`、`涉及文档` 必须存在且有内容；`范围与约束` 可选。
2. 每段保持简短。不得把背景、目标、实现步骤、约束和交付物堆成一个长段落。
3. `目标` 只写结果，不写“先……再……”的执行清单；实现步骤应拆成子任务。
4. `涉及文档` 逐条写项目内路径或外部链接并说明用途；没有时写 `- 无`，不得省略标题。
5. 验收条件继续写入 `acceptanceCriteria`，不得复制进 description 充当结构化描述。
6. 对旧任务执行领取、拆分或实质修改时，先根据现有任务、父任务和项目文档整理成该结构；证据不足时询问用户，不得编造背景或文档路径。
7. Vault 中按 Markdown 行保存 `description`，保留标题和列表结构，确保任务详情可直接渲染阅读。

## 领取和推进任务

1. 确认任务描述满足“背景、目标、涉及文档”结构，并且任务是满足 [递归拆解协议](references/decomposition-protocol.md) 的叶子；描述不合格、仍可继续拆分或已有子任务时不得领取父任务。
...
3. 校验完成任务的 Gate 与证据。
4. 校验所有新建或实质修改任务的 description 均包含非空的“背景、目标、涉及文档”Markdown 段落。
5. 对新计划执行递归拆解自检：单一结果根、父子引用、叶子准入、测试覆盖和无环依赖均通过。
6. 执行 `git diff --check -- .clawpm`、`git status --short -- .clawpm` 和 `git diff -- .clawpm`。
7. 向用户报告创建、拆分、领取、测试或完成了哪些任务，以及验证结果。
