      ...
      <div className="mx-auto max-w-5xl space-y-5 px-6 py-8">
        ...
        {query.isLoading ? (
          <section className="flex min-h-64 flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white px-6 shadow-sm">
            ...
            <p className="mt-4 text-sm font-medium text-slate-700">正在准备安装…</p>
          </section>
        ) : query.isError ? (
          ...
          <section className="flex min-h-64 flex-col items-center justify-center rounded-2xl border border-emerald-200 bg-gradient-to-b from-emerald-50 to-white px-6 text-center shadow-sm">
            ...
            <h2 className="mt-4 text-xl font-semibold text-slate-900">已经装好了</h2>
            <p className="mt-2 text-sm text-slate-500">Claude、Cursor、Codex 和 CodeBuddy 都可以直接使用 ClawPM 工作流。</p>
        ...
        <details className="group overflow-hidden rounded-xl border border-slate-200 bg-white">
          <summary className="flex cursor-pointer list-none items-center justify-between px-4 py-4 text-sm text-slate-600 hover:bg-slate-50">
            <span className="flex items-center gap-2">
              <Settings2 className="h-4 w-4 text-slate-400" />
              高级设置
              <span className="text-xs text-slate-400">通常不需要打开</span>
            </span>
          ...
          <div className="border-t border-slate-100 bg-slate-50/60 p-4">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                <h2 className="text-sm font-semibold text-slate-800">按平台或项目单独配置</h2>
                <p className="mt-1 text-xs text-slate-500">只有需要把 Skill 随 Git 分享给团队时，才使用“仅当前项目”。</p>
              </div>
              <button
                type="button"
                onClick={() => void query.refetch()}
                disabled={Boolean(running) || query.isFetching}
                className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs text-slate-600 hover:border-indigo-200 hover:text-indigo-600 disabled:opacity-50"
              >
                ...
                return (
                  <article key={platform} className="overflow-hidden rounded-xl border border-slate-200 bg-white">
                    <div className="flex items-center gap-3 border-b border-slate-100 px-4 py-3">
                      ...
                      <div>
                        <h3 className="text-sm font-semibold text-slate-900">{first?.platformName ?? platform}</h3>
                        <p className="text-[11px] text-slate-400">自动维护，无需手工复制文件</p>
                      </div>
                    </div>
                          ...
                          <div key={target.scope} className="space-y-2.5 px-4 py-3">
                            <div className="flex items-center justify-between gap-3">
                              <div className="flex items-center gap-2 text-xs font-medium text-slate-700">
                                {target.scope === 'user' ? <UserRound className="h-3.5 w-3.5 text-slate-400" /> : <FolderGit2 className="h-3.5 w-3.5 text-slate-400" />}
