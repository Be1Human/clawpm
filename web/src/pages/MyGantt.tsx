...
const STATUS_LABEL: Record<string, string> = {
  backlog: '未排期', planned: '未开始', active: '进行中', review: '验收中', done: '已完成',
};
...
function fmtDate(d: Date) { return d.toLocaleDateString('zh-CN', { month: 'short', day: 'numeric' }); }
...
export default function MyGantt() {
  ...
  const groups = useMemo(() => {
    ...
    for (const t of tasks) {
      const key = groupBy === 'domain' ? (t.domain?.name || '未分组') : (STATUS_LABEL[t.status] || t.status);
      if (!map[key]) map[key] = [];
      map[key].push(t);
    }
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-6 py-3 border-b border-gray-200 bg-white flex-shrink-0">
        <div className="flex items-center gap-2">
          <h1 className="text-lg font-semibold text-gray-900 mr-4">我的甘特图</h1>
          <span className="text-gray-400 text-sm">分组：</span>
          ...
          <span className="text-gray-400 text-sm">粒度：</span>
